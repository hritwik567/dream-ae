...
#define CONFIG_FILE_DEFAULT CURRENT_DIR "../config_dramsim3/DDR5_32Gb_x4_4800_norfm.ini"
...
uns64       INST_LIMIT      = 1000*1000*1000; // set default 
uns64       NUM_THREADS     = 0;
...
uns64       tDRFM=410*4;   //-- DRFM latency is same as tRFC

uns64       MCPARA_DRFM_PERIOD=0; // average ACTs per DRAM for MC-PARA (40 for p=1/40)
...
int         num_threads = 0;
std::string DRAMSIM3CFG = CONFIG_FILE_DEFAULT;
...
void die_usage() {
    ...
    printf("               -l3sizemb    <num>    Set L3  Cache size to <num> MB (Default: 8MB)\n");
  ...
  int ii;
	...
	if (argv[ii][0] == '-') {	    
	  ...
	  //-- legacy
	   ...
	   else if (!strcmp(argv[ii], "-mcpara")) {
		if (ii < argc - 1) {		  
		    MCPARA_DRFM_PERIOD = atoi(argv[ii+1]);
		    ii += 1;
		}
	   }
	  ...
	  //-- legacy
	    ...
	    else if (!strcmp(argv[ii], "-dramsim3cfg")) {
			if (ii < argc - 1) {	
			    char cwd[1024];  // Buffer to store the current working directory
				if (getcwd(cwd, sizeof(cwd)) != nullptr) {
					DRAMSIM3CFG = std::string(cwd) + "/" + std::string(argv[ii + 1]);
				} else {
					printf("getcwd() error");
				}
				ii += 1;
		...
		else if (!strcmp(argv[ii], "-memsize")) {
			if (ii < argc - 1) {
				MEM_SIZE_MB = atoi(argv[ii + 1]);
				ii += 1;
			}
		}
