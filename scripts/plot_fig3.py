...
data = pd.read_csv('stats/fig3.csv')
...
axs[0].bar([i + 2*bar_width for i in index], slowdown(data['mop4_para_drfmab_eager']), bar_width, label='DRFMab', edgecolor='black', color=colors[2], zorder=2)
...
axs[0].set_yticks(list(range(0, ymax+1, 10)))
axs[0].grid(axis='y', linestyle='-', alpha=0.6, zorder=-10)
axs[0].get_xticklabels()[-1].set_weight('bold')

axs[0].legend(ncol=5, bbox_to_anchor=(0.5, 0.88), loc='center', prop={'weight': 'bold'}, borderpad=0.2, handletextpad=0.2, columnspacing=1.2, edgecolor='none', facecolor='none')
...
axs[1].bar([i + 2*bar_width for i in index], slowdown(data['mop4_mint_drfmab_eager']), bar_width, label='DRFMab', edgecolor='black', color=colors[2], zorder=2)
...
axs[1].set_yticks(list(range(0, ymax+1, 10)))
...
plt.savefig('fig3.pdf', bbox_inches='tight', pad_inches=0.05)